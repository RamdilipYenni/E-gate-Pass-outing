...
<div class="table">
	<table text align="center">
		<tr><th>id </th> <th>Studentname</th> <th>branch</th> <th>rollnumber</th> <th>Year</th>
			...
			<th>reason</th> <th>print</th> <th>delete</th></tr>
		<?PHP
		include 'connect.php';
		$query = "SELECT * FROM `outpass` ORDER BY `sno` DESC";
		$sql = mysqli_query($conn, $query);
			while($row = mysqli_fetch_array($sql))
			echo "<tr><td>".$row['sno']."</td><td>".$row['studentname']."</td><td>".$row['branch']."</td><td>".$row['roll']."</td><td>".$row['studying']."</td><td>".$row['leaving']."</td><td>".$row['arraival']."</td><td>".$row['place']."</td><td>".$row['phone']."</td><td>".$row['reason']."</td><td><a href='print.php?q=".$row['sno']."'target='blank'>Print</a></td><td><a href='delete.php?q=".$row['sno']."'target='blank'>delete</a></td> </tr>";
		?>
	</table>
